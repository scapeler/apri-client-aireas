...
//			}
			...
			, loadData:	function (layer, param) {
				...
				if (agrTypesA == undefined) {
					initDropDownPestinfo({container:mapPanes.controlerPane});
				};
...
//						//map.panTo(eindhovenLatLng);
	...
	var agrTypesA, 		agrTypeASelected, 	dropDownAgrTypeListAContainer, dropDownAgrTypeListADispatchers; 
	var agrJaren, 		agrJaarSelected, 	dropDownAgrJaarListContainer, dropDownAgrJaarListDispatchers; 
	var agrStoffenA, 	agrStofASelected, dropDownAgrStofListAContainer, dropDownAgrStofListADispatchers; 
	...
	var initDropDownPestinfo = function(options) {
		...
		*/
		...
		agrJaren 	= d3.map( {"2014":{id:"2014",name:"2014"},"2015":{id:"2015",name:"2015"}} ); 
		agrTypesA 	= d3.map( {"aang":{id:"aang",name:"aantal aangetroffen"},"avg":{id:"avg",name:"Gemiddelde"}} ); 
		agrStoffenA = d3.map( {"imdcpd":{id:"imdcpd",name:"imidacloprid"},"test":{id:"test",name:"test"}} ); 
		...
		dropDownAgrTypeListAContainer 			= document.createElement('div');
		dropDownAgrTypeListAContainer.id 		= APRI.UTIL.apriGuid(dropDownAgrTypeListAContainer.id );
		dropDownAgrTypeListAContainer.className = dropDownAgrTypeListAContainer.className + " leaflet-control pestinfo-selector";
		dropDownPestinfoContainer.appendChild(dropDownAgrTypeListAContainer);

		dropDownAgrJaarListContainer 			= document.createElement('div');
		dropDownAgrJaarListContainer.id 		= APRI.UTIL.apriGuid(dropDownAgrJaarListContainer.id );
		dropDownAgrJaarListContainer.className	= dropDownAgrJaarListContainer.className + " leaflet-control pestinfo-selector";
		dropDownPestinfoContainer.appendChild(dropDownAgrJaarListContainer);

		dropDownAgrStofListAContainer 			= document.createElement('div');
		dropDownAgrStofListAContainer.id 		= APRI.UTIL.apriGuid(dropDownAgrStofListAContainer.id );
		dropDownAgrStofListAContainer.className	= dropDownAgrStofListAContainer.className + " leaflet-control pestinfo-selector";
		dropDownPestinfoContainer.appendChild(dropDownAgrStofListAContainer);

		container.appendChild(dropDownPestinfoContainer);
		
		// set default
		if (areas[currentArea].deepLinkVars.agrJaar == undefined) {
			areas[currentArea].deepLinkVars.agrTypeListA 	= 'JGMKNnon';
			areas[currentArea].deepLinkVars.agrJaar 		= '2015';
			areas[currentArea].deepLinkVars.agrStofListA 	= 'imacl';
		}
		
		var currentAgrJaar					= areas[currentArea].deepLinkVars.agrJaar;
		var currentAgrTypeListA				= areas[currentArea].deepLinkVars.agrTypeListA;
		var currentAgrStofListA				= areas[currentArea].deepLinkVars.agrStofListA;
						
		dropDownAgrTypeListADispatchers		= d3.dispatch("load", "statechange");
		dropDownAgrJaarListDispatchers		= d3.dispatch("load", "statechange");
		dropDownAgrStofListADispatchers		= d3.dispatch("load", "statechange");

			
		dropDownAgrTypeListADispatchers.on("load.menu", function(agrTypesA) {
  			agrTypeASelected = d3.select("#"+dropDownAgrTypeListAContainer.id)
    			.append("div")
    			.append("select")
      			.on("change", function() { 
					dropDownAgrTypeListADispatchers.statechange(agrTypesA.get(this.value));
				});
			agrTypeASelected.selectAll("option")
      			.data(agrTypesA.values())
    			.enter().append("option")
      			.attr("value", function(d) { 
					return d.name; 
					})
      			.text(function(d) { return d.name; });
			dropDownAgrTypeListADispatchers.on("statechange.menu", function(opt) {
				if (opt == undefined) return;
				var _agrType = opt.name;
				areas[currentArea].deepLinkVars.agrType = _agrType;
				setPushState();
    			agrTypeASelected.property("value", _agrType);
				currentAgrTypeListA = areas[currentArea].deepLinkVars._agrType;
		...
		dropDownAgrJaarListDispatchers.on("load.menu", function(agrJaren) {
  			agrJaarSelected = d3.select("#"+dropDownAgrJaarListContainer.id)
    			.append("div")
    			.append("select")
      			.on("change", function() { 
					dropDownAgrJaarListDispatchers.statechange(agrJaren.get(this.value));
				});
			agrJaarSelected.selectAll("option")
      			.data(agrJaren.values())
    			.enter().append("option")
      			.attr("value", function(d) { 
					return d.name; 
					})
      			.text(function(d) { return d.name; });
			dropDownAgrJaarListDispatchers.on("statechange.menu", function(opt) {
				if (opt == undefined) return;
				var _agrJaar = opt.name;
				areas[currentArea].deepLinkVars.agrJaar = _agrJaar;
				setPushState();
    			agrJaarSelected.property("value", _agrJaar);
				currentAgrJaar = areas[currentArea].deepLinkVars._agrJaar;
		...
		dropDownAgrStofListADispatchers.on("load.menu", function(agrStoffenA) {
  			agrStofASelected = d3.select("#"+dropDownAgrStofListAContainer.id)
    			.append("div")
    			.append("select")
      			.on("change", function() { 
					dropDownAgrStofListADispatchers.statechange(agrStoffenA.get(this.value));
				});
			agrStofASelected.selectAll("option")
      			.data(agrStoffenA.values())
    			.enter().append("option")
      			.attr("value", function(d) { 
					return d.name; 
					})
      			.text(function(d) { return d.name; });
			dropDownAgrStofListADispatchers.on("statechange.menu", function(opt) {
				if (opt == undefined) return;
				var _agrStof = opt.name;
				areas[currentArea].deepLinkVars.agrStof = _agrStof;
				setPushState();
    			agrStofASelected.property("value", _agrStof);
				currentAgrStofListA = areas[currentArea].deepLinkVars._agrStof;
		...
		dropDownAgrTypeListADispatchers.load(agrTypesA);
  		dropDownAgrTypeListADispatchers.statechange(
			agrTypesA.get(areas[currentArea].deepLinkVars.agrJaar)
		);  

		dropDownAgrJaarListDispatchers.load(agrJaren);  
  		dropDownAgrJaarListDispatchers.statechange(
			agrJaren.get(areas[currentArea].deepLinkVars.agrTypeListA)
		);  

		dropDownAgrStofListADispatchers.load(agrStoffenA); 
  		dropDownAgrStofListADispatchers.statechange(
			agrStoffenA.get(areas[currentArea].deepLinkVars.agrStofListA)
		);  
