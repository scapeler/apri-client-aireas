...
//						//map.panTo(eindhovenLatLng);
	...
	var initDropDownPestinfo = function(options) {
		...
		*/
		...
		piAgrTypes 	= d3.map( {	"aang":{id:"aang",name:"aantal aangetroffen"}
								,"avg":{id:"avg",name:"Gemiddelde"}
								,"JGMKNnon":{id:"JGMKNnon",name:"JGMKNnon"}} ); 
		piAquoCodes = d3.map( {	"imdcpd":{id:"imdcpd",name:"imidacloprid"}
		...
		if (areas[currentArea].deepLinkVars.piAgrJaar == undefined) {
			...
			areas[currentArea].deepLinkVars.piAquoCode 	= 'imacl';
		}
		...
		dropDownPiAgrJaarListDispatchers.on("load.menu", function(piAgrJaren) {
			...
			dropDownPiAgrJaarListDispatchers.on("statechange.menu", function(opt) {
				...
				//todo: activate layer
				layerObjects['pestInfoStofKentallen'].options.loadData(options.layer,{piAgrJaar:currentPiAgrJaar,piAquoCode:currentPiAquoCode,piAgrType:currentPiAgrType});
