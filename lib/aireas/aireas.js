...
//		viewCoordinates: [51.45401, 5.47668],
	...
	var aireasAvgType = 'UFP'; // 'UFP', 'PM1', 'PM25', 'PM10', 'SPMI', 'OZON', 'NO2', 'CELC', 'HUM', 'AMBHUM', 'AMBTEMP'
...
//	var areaVars;
	...
	var aireasLogoClick = function(e) {
		...
		addRemoveLayerToMap('aireasGridGemLayer', currentArea);
	}		
...
//			this.style.color = 'white';
	...
	var areaLayerPackages = {
		'aireas': [ 
			{layer:'aireasGridGemLayer',				group: 'AiREAS', sortId: 1, sensor: 'UFP', 	label:'Sensor UFP'},
			{layer:'aireasGridGemLayer1',				group: 'AiREAS', sortId: 1, sensor: 'PM1', 	label:'Sensor PM1' },
			{layer:'aireasGridGemLayer2',				group: 'AiREAS', sortId: 1, sensor: 'PM25', label:'Sensor PM2.5' },
			{layer:'aireasGridGemLayer3',				group: 'AiREAS', sortId: 1, sensor: 'PM10', label:'Sensor PM10' },
			{layer:'aireasGridGemLayer4',				group: 'AiREAS', sortId: 1, sensor: 'NO2', 	label:'Sensor NO2' },
			{layer:'aireasGridGemLayer5',				group: 'AiREAS', sortId: 1, sensor: 'OZON', label:'Sensor OZON' },
			{layer:'aireasGridGemLayer6',				group: 'AiREAS', sortId: 1, sensor: 'HUM', 	label:'Sensor Hum' },
			{layer:'aireasGridGemLayer7',				group: 'AiREAS', sortId: 1, sensor: 'CELC', label:'Sensor Celc' },
			{layer:'aireasTileLayer',					group: 'AiREAS', sortId: 1, label:'Tilelayer' },
			{layer:'aireasMeetkastenLayer',				group: 'AiREAS', sortId: 1, label:'Airboxen' },
			{layer:'aireasWMSPuntMetingenLayer',		group: 'AiREAS', sortId: 1, label:'Puntmetingen' },
			{layer:'aireasGridGemStatisticsLayer',		group: 'AiREAS', sortId: 1, label:'Statistiek' },
			{layer:'aireasGridGemStatisticsAvgLayer',	group: 'AiREAS', sortId: 1, label:'Statistiek gem.' }
		],
		'cbs': { 
		...
		'ndw': { 
			'ndwTrafficFlowLayer':			{group: 'NDW', sortId: 1 },
	...
	var areaLayersPackagesList = {
		  'EHV': { layerPackages: {'aireas': true} } //,	'ndw': true, 'ehv': true, 	'ov': true, 'divers': true, 'cbs': true } }
		...
		if (areaLayersPackagesList[areaCode] != undefined ) {
			for (var areaLayerPackageKey in areaLayersPackagesList[areaCode].layerPackages) {
				...
				for (var layerKey in _package ) {
					var _layer = initAreaLayer(layerKey);
					areasArea.layers[_layer.layer] = _layer;
				}
			}
		} 
	}
	...
	var activateLayer = function(layer, areaIndex ) {
		if (areas[areaIndex].layers[layer] != undefined ) {
			...
			if (layerClasses[layer] == undefined || layerClasses[layer].layerClass == undefined) {
				...
				if (areas[areaIndex].layers[layer].object == undefined) areas[areaIndex].layers[layer].object = layerClasses[layer].layerClass();
				areas[areaIndex].layers[layer].activate = true;
				areas[areaIndex].layers[layer].active = false;
				addLayersToMap();
			}
		} else {
			//alert('Layer '+layer+' for area '+areaIndex+' is not defined. Skipping' );
			areas[areaIndex].layers[layer] = initAreaLayer(layer);
			checkLayerClass(layer);
	...
	var addLayersToMap = function() {
		for (var i=0;i<areas.length;i++) {
			var _area = areas[i];
			for (var key in _area.layers) {
			//	var _layer = _area.layers[key];
				if (_area.layers[key].active == false && _area.layers[key].activate == true && _area.layers[key].object ) {
					map.addLayer(_area.layers[key].object);
					_area.layers[key].activate = false;
					_area.layers[key].active = true;
				}
			}
		}
	}
...
//		map 			= apriLeafLetBase.createMap(this.container, {viewCoordinates: [appConfig.mapCenterLat, appConfig.mapCenterLng], viewZoomLevel: appConfig.viewZoomLevel });
		map 			= apriLeafLetBase.createMap(leafletContainer, {'topAreaHeight':85, viewCoordinates: [appConfig.mapCenterLat, appConfig.mapCenterLng], viewZoomLevel: appConfig.viewZoomLevel
		...
		videoCameraLayer = new L.geoJson(videoCamera_GeoJson
			, {
                    ...
                    , pointToLayer: function (feature, latlng) {
                        return L.marker(latlng, { icon: videoCameraMarker} );
                        }
                }
		);
...
*/
		...
		participationLayer = new L.geoJson(participation_GeoJson
			, {
                    ...
                    , pointToLayer: function (feature, latlng) {
                        return L.marker(latlng, { icon: usersMarker} );
                        }
                }
		);
...
Op ons bedrijf houden wij een 600 melkgeiten. De melk van onze geiten wordt verwerkt tot kaas, ijs en likeur, welke wij verkopen in onze winkel.",
		...
		localProductsLayer = new L.geoJson(localProducts_GeoJson
			, {
                    ...
                    , pointToLayer: function (feature, latlng) {
                        return L.marker(latlng, { icon: localProductMarker} );
                        }
                }
		);
...
met 1,5% per jaar gestegen.",
		...
		solutionsLayer = new L.geoJson(solutions_GeoJson
			, {
                    ...
                    , pointToLayer: function (feature, latlng) {
                        return L.marker(latlng, { icon: solutionMarker} );
                        }
                }
		);
		...
		cbsBuurtProjectEhvAirportLayer = new L.geoJson(null, {
			...
			, loadDataOnce:
				function () {
					var mapCenter = map.getCenter();
//					ajaxGetData(APRI.getConfig().urlSystemRoot + "/data/aireas/getCbsBuurtInfo/*", 
					var _url= "http://localhost:4000/SCAPE604/openiod?SERVICE=WPS&REQUEST=Execute&identifier=transform_observation&inputformat=projectEhvAirport&objectid=areas&format=xml"
...
//		heatmapTrafficSpeedLayer = L.heatLayer(trafficSpeedDataRecords, {radius: 15, blur:10, max:1, gradient: {0.4: 'blue', 0.6: 'cyan', 0.7: 'lime', 0.8: 'yellow', 1: 'red'}, loadTrafficData: "SPEED", loadDataTrafficOnce: loadDataTrafficOnce}); //, max:10, gradient: {0.4: 'blue', 0.65: 'lime', 1: 'red'}
		...
		function loadDataTrafficFlowLayer(layer, options) {
			...
			for (var i=0; i<retrievedDateCacheTrafficFlow.length;i++) {
				var diff = retrievedDateCacheTrafficFlow[i].retrievedDateDate.getTime() - new Date(options.retrievedDateMax).getTime();
				if (diff > -480000 && diff < 480000) {  //less than 9min = hit
					retrievedDateMax = retrievedDateCacheTrafficFlow[i].retrievedDate;
					//console.log('retrieveddate (cache/hit): ' + retrievedDateMax + ' '+options.retrievedDateMax);
					window.setTimeout(function(){
						heatmapTrafficFlowLayer.setLatLngs(retrievedDateCacheTrafficFlow[i].trafficFlowDataRecords);
...
// ===============  OV WMS Agency Shapes end
        ...
        EnkelbestemmingLayer = L.tileLayer.wms(siteProtocol+sitePrefixExtern+"afnemers.ruimtelijkeplannen.nl/afnemers/services/wms", {
        // ?service=WMS&version=1.1.0&bbox=123657.03125,499459.625,129679.1328125,504556.78125&width=512&height=433&srs=EPSG:28992&format=
...
//===========================================================================  end of constructor
	...
	var aireasGridGemLayer, aireasHistGridGemLayer, cbsGridGemLayer, cbsGridGemEnvelopeLayer, aireasTileLayer, openBasisKaartLayer, PDOK_BAG_pand_Layer, pdokLuchtfotoLayer,
		videoCameraLayer, participationLayer, solutionsLayer, parkingLayer, localProductsLayer,
		cbsBuurtInfoLayer, cbsBuurtRookRisicoInfoLayer, cbsBuurtProjectEhvAirportLayer,
		ndwTrafficFlowLayer, ndwTrafficSpeedLayer, ndwTrafficSpeedLayerTest, aardbevingenLocationsLayer, heatmapTrafficFlowLayer, //heatmapTrafficSpeedLayer,
		nslSegmentEindhovenWMSLayer, nslResultEindhovenWMSLayer, nslReceptorEindhovenWMSLayer, nslMeasureEindhovenWMSLayer, dankPm10InvangWMSLayer, dankWandelWMSLayer, dankFietsWMSLayer, dankLandBelevingWMSLayer, dankBodemMaatregelCTotaalWMSLayer,
		ovStopsWMSLayer, ovShapesWMSLayer,
...
//        	+ ':' + pad(tzo % 60);
	...
	//	if (layer == 'aireasGridGemLayer') {
		...
		layerClasses[layer] = {};
		layerClasses[layer].layerClass = function() {
...
//					ajaxGetData2(APRI.getConfig().urlSystemRoot + "/data/aireas/getCbsGemInfo/*",
	...
	var initCbsGridGemEnvelopeLayer = function(geom, centroid) {
...
//				}
		...
		var _bounds = cbsGridGemEnvelopeLayer.getBounds();
...
//		map.setMaxBounds(_bounds);
	...
	function playSliderMovie(layer, startDate, avgType) {
		var next=false;
		if (startDate.getTime() > new Date().getTime()) next = false
		else next = true;
